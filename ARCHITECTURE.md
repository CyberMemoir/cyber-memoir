...
- 分数下限（ADR 0004）：重排分数低于 `answer_score_floor` 的 chunk 不能支撑回答，落到弃答路径。
  RRF 分数编码名次而非匹配质量，不承载阈值；精确别名是同一性匹配，不受该阈值约束。
  检索接口只暴露 `retrieval_score` 与 `scores_calibrated`，不做过滤，弃答由回答接口负责。
- 融合常量（RRF k 与权重、各通道条数、单 Source 限额、候选上限）与分数下限均为设置项，供金标准评测集扫参。
...
4. **事实锁定的 RAG**：V1 使用 LLM 选择已审核断言、确定性组装回答，不任意改写。模型失败回退到引用型摘录。
   弃答同时受断言可得性与检索分数下限约束；没有校准打分器时显式标记 `score_floor_unenforced`，不静默放行。
